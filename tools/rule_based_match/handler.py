...
# Import address normalization for site-level scoring
try:
    from tools.rule_based_match.address_normalization import normalize_address
except ImportError:
...
# ---------------------------------------------------------------------------
# SITE scoring weights (address-focused, cumulative)
# ---------------------------------------------------------------------------
SITE_WEIGHT_ADDRESS_LINE1_JW = 90
SITE_WEIGHT_ADDRESS_LINE2_JW = 25
SITE_WEIGHT_CITY_JW = 30
SITE_WEIGHT_POSTAL_CODE = 55
SITE_WEIGHT_COUNTRY = 40
SITE_WEIGHT_COUNTY_JW = 15
SITE_WEIGHT_OPERATING_UNIT = 20

SITE_JW_THRESHOLD_ADDRESS = 0.80
SITE_JW_THRESHOLD_CITY = 0.85
SITE_JW_THRESHOLD_COUNTY = 0.85

SITE_MATCH_THRESHOLD = 120
SITE_HIGH_CONFIDENCE = 200
SITE_MAX_POSSIBLE = 275
...
# ---------------------------------------------------------------------------
# SITE scoring (address-focused, cumulative)
# ---------------------------------------------------------------------------
def _score_site_pair(incoming: dict, candidate: dict) -> dict:
    """Compute cumulative match score for SITE records.

    Address-focused scoring using Jaro-Winkler on normalized address fields
    and exact match on postalCode, country, and operatingUnit.
    """
    cumulative = 0
    contributing = []

    # addressLine1 — Jaro-Winkler on normalized (90 pts if JW >= 0.80)
    inc_addr1 = normalize_address(incoming.get("addressLine1", "") or "")
    cand_addr1 = normalize_address(candidate.get("addressLine1", "") or "")
    if inc_addr1 and cand_addr1:
        jw = jellyfish.jaro_winkler_similarity(inc_addr1, cand_addr1)
        if jw >= SITE_JW_THRESHOLD_ADDRESS:
            cumulative += SITE_WEIGHT_ADDRESS_LINE1_JW
            contributing.append(f"addressLine1_jw({jw:.3f})")

    # addressLine2 — Jaro-Winkler on normalized (25 pts if JW >= 0.80)
    inc_addr2 = normalize_address(incoming.get("addressLine2", "") or "")
    cand_addr2 = normalize_address(candidate.get("addressLine2", "") or "")
    if inc_addr2 and cand_addr2:
        jw = jellyfish.jaro_winkler_similarity(inc_addr2, cand_addr2)
        if jw >= SITE_JW_THRESHOLD_ADDRESS:
            cumulative += SITE_WEIGHT_ADDRESS_LINE2_JW
            contributing.append(f"addressLine2_jw({jw:.3f})")

    # city — Jaro-Winkler on normalized (30 pts if JW >= 0.85)
    inc_city = normalize_address(incoming.get("city", "") or "")
    cand_city = normalize_address(candidate.get("city", "") or "")
    if inc_city and cand_city:
        jw = jellyfish.jaro_winkler_similarity(inc_city, cand_city)
        if jw >= SITE_JW_THRESHOLD_CITY:
            cumulative += SITE_WEIGHT_CITY_JW
            contributing.append(f"city_jw({jw:.3f})")

    # postalCode — exact match, case-insensitive, strip whitespace (55 pts)
    inc_pc = _safe_lower(incoming.get("postalCode"))
    cand_pc = _safe_lower(candidate.get("postalCode"))
    if inc_pc and cand_pc and inc_pc == cand_pc:
        cumulative += SITE_WEIGHT_POSTAL_CODE
        contributing.append("postalCode_exact")

    # country — exact match, case-insensitive (40 pts)
    inc_country = _safe_lower(incoming.get("country"))
    cand_country = _safe_lower(candidate.get("country"))
    if inc_country and cand_country and inc_country == cand_country:
        cumulative += SITE_WEIGHT_COUNTRY
        contributing.append("country_exact")

    # county — Jaro-Winkler on normalized (15 pts if JW >= 0.85)
    inc_county = normalize_address(incoming.get("county", "") or "")
    cand_county = normalize_address(candidate.get("county", "") or "")
    if inc_county and cand_county:
        jw = jellyfish.jaro_winkler_similarity(inc_county, cand_county)
        if jw >= SITE_JW_THRESHOLD_COUNTY:
            cumulative += SITE_WEIGHT_COUNTY_JW
            contributing.append(f"county_jw({jw:.3f})")

    # operatingUnit — exact match, case-insensitive (20 pts)
    inc_ou = _safe_lower(incoming.get("operatingUnit"))
    cand_ou = _safe_lower(candidate.get("operatingUnit"))
    if inc_ou and cand_ou and inc_ou == cand_ou:
        cumulative += SITE_WEIGHT_OPERATING_UNIT
        contributing.append("operatingUnit_exact")

    # Normalized score (cumulative / max_possible)
    normalized = round(cumulative / SITE_MAX_POSSIBLE, 4) if SITE_MAX_POSSIBLE > 0 else 0.0

    # isDefinitive: high confidence (>= 200) or clearly not a match (< 80)
    is_definitive = cumulative >= SITE_HIGH_CONFIDENCE or cumulative < 80

    return {
        "candidateId": candidate.get("siteId", ""),
        "ruleBasedScore": normalized,
        "cumulativeScore": cumulative,
        "maxPossibleScore": SITE_MAX_POSSIBLE,
        "contributingFields": contributing,
        "isDefinitive": is_definitive,
        "partyType": "SITE",
        "scoreType": "cumulative",
...
def handler(event, context):
    ...
    if party_type == "ORGANIZATION":
        ...
        results = [_score_site_pair(incoming, c) for c in candidates]
    else:
